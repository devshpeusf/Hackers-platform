...
/**
 * The application flow, built from the PLAT-35 design canvas.
 *
...
export default function ApplyPage() {
  ...
  if (step === 0) return <SignIn onStart={() => setStep(1)} />;
  if (step === DONE) return <Submitted onReplay={() => setStep(0)} />;
...
function DiscordButton({ onClick, className }: { onClick?: () => void; className?: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={clsx(
        "pixel-btn-solid inline-flex items-center justify-center gap-2.5 px-7 py-[15px] font-body text-xs font-bold tracking-wide text-surface-bg",
        className,
      )}
    >
      <DiscordIcon size={18} />
...
/* ------------------------------------------------------------------ */
/* Step 0 — sign in (board A from the design canvas)                   */
/* ------------------------------------------------------------------ */

function SignIn({ onStart }: { onStart: () => void }) {
  return (
  ...
  return (
    <div className={clsx("flex min-h-screen flex-col md:flex-row", STARFIELD)}>
      ...
      <aside className="hidden w-[262px] shrink-0 flex-col border-r-4 border-border-default bg-surface-sidebar px-[18px] py-7 md:flex">
        ...
        />
        <div className="text-[10px] tracking-wide text-text-faintest">MREYES &middot; DISCORD ACCOUNT</div>
      </aside>
      ...
      <>
        <div className="flex flex-col gap-[18px] sm:flex-row">
          <div className="flex-1">
            ...
            <input className={FIELD} defaultValue="Mariana" />
          </div>
          <div className="flex-1">
            <Label>Last Name</Label>
            <input className={FIELD} defaultValue="Reyes" />
          </div>
        </div>
        <div>
          <Label>Email</Label>
          <input className={FIELD} type="email" defaultValue="mariana@usf.edu" />
        </div>
        <div className="flex flex-col gap-[18px] sm:flex-row">
          <div className="flex-1">
            <Label>Phone</Label>
          ...
          <div className="flex-1">
            ...
            <div className="flex w-full items-center gap-2 border border-text-primary/7 bg-surface-bg/35 px-[14px] py-3 text-[13px] text-text-faintest">
              <DiscordIcon size={14} />
              mreyes
            </div>
          </div>
  ...
  if (step === 3) {
    return (
      <>
        <div>
          <Label>School</Label>
          <input className={FIELD} defaultValue="South Fl" />
          <div className="mt-1.5 bg-surface-bg shadow-[0_-2px_0_0_var(--color-accent-pink-light),0_2px_0_0_var(--color-accent-pink-light),-2px_0_0_0_var(--color-accent-pink-light),2px_0_0_0_var(--color-accent-pink-light)]">
            {schoolSuggestions.map((s, i) => (
              <div
                key={s.name}
                className={clsx(
                  "flex justify-between px-3.5 py-2.5 text-[13px]",
                  i === 0 ? "bg-accent-pink-light/10 text-accent-pink-light" : "border-t border-text-primary/6 text-text-secondary",
                )}
              >
                <span>{s.name}</span>
                {s.note && <span className="text-[10px] text-text-faintest">{s.note}</span>}
              </div>
            ))}
            <div className="border-t border-text-primary/6 px-3.5 py-2.5 text-xs italic text-text-dim">
              Not listed? Enter it manually &mdash;{" "}
              <span className="text-accent-amber">needs a decision (PLAT-15)</span>
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-[18px] sm:flex-row">
...
function Submitted({ onReplay }: { onReplay: () => void }) {
  return (
      ...
      <div className="terminal-window w-full max-w-[620px]">
        ...
        <div className="relative px-7 pb-7 pt-8 text-center">
          ...
          <p className="mb-5 text-[13px] leading-[1.8] text-text-muted">
            We&apos;ll email <span className="text-accent-teal">mariana@usf.edu</span> when decisions
            go out.
          </p>
